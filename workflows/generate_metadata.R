...
generate_metadata <- function(forecast_file, metadata_yaml, forecast_issue_time, forecast_iteration_id, forecast_file_name_base){
  ...
  if(theme == "terrestrial_daily"){
    ...
    attributes <- tibble::tribble(
      ...
      "statistic",          "[dimension]{calculated statistic}",      "dimensionless",         NA,            "statistic calculated from ensemble members",          "character",
      "siteID",             "[dimension]{neon site}",                     "dimensionless",      NA,           "NEON site ID",  "character",
  ...
  dataset <- eml$dataset(
    title = "CABLE forecast for nee, le and vswc",
