...
if __name__ == "__main__":
    ...
    for siteID in siteID_list:
        print("Generating combined met file for "+siteID)
        # Calculate observation date
        forecast_datetime = datetime.strptime(forecast_date,'%Y-%m-%d')
        obs_datetime = forecast_datetime - timedelta(days=1)
        obs_date = datetime.strftime(obs_datetime,'%Y-%m-%d')

        # Define raw file name
        fname = "data/NEON/raw/NEONMetFile_"+siteID+"_"+obs_date+".csv"
